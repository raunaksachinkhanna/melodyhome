...
| Artisan Community | ठठेरा craftsmen and women in Amritsar producing made-to-order items | Timely production briefs with quantity, SKU, and lead time |
| Delivery Partner | Shiprocket-assigned courier responsible for physical dispatch and delivery | Accurate AWB generation, pickup scheduling, and address data |

**4.2 Usage Scenarios**

**Scenario 1: Individual Purchase**
A customer discovers Melody Home on Instagram, visits melodyhome.in, browses the brass catalogue, and adds the Aśva Stirrer (set of 2, ₹1,650) to cart. The system displays a lead time notice: made to order, dispatched within 7 working days (similar to ordering food on Uber Eats or ordering a product from Amazon). The customer completes checkout via Razorpay UPI. On payment confirmation, the system writes the order to the database, queues a WhatsApp notification, triggers Shiprocket courier assignment, and generates a production order for the artisan team which takes place within 30 seconds. The customer receives a WhatsApp message with order confirmation. When dispatched, they receive a second message with the Shiprocket tracking link. No human intervention required.
...
Four weeks before Diwali, the AI demand forecasting module analyses the previous year's order patterns and predicts a 6x spike in brass product orders. The system automatically generates a production brief for the artisan team: estimated units per SKU, materials required, recommended start date. Tannya reviews and confirms. Artisans begin production before orders arrive which eliminate the reactive scramble of previous years.
